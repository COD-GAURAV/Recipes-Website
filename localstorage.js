  ...
  .then((data) => {
      ...
      Array.from(recipeCards).forEach((card) => {
        card.addEventListener("click", (e) => {
          if (e.target.classList.contains("view-btn")) {
            ...
            document.querySelector(".recipe-making-container").style.display =
              "flex";
    ...
    input.addEventListener("input", (e) => {
      ...
      let filter = dataInArray.filter((val) => {
        return val.name.toLowerCase().startsWith(input.value.toLowerCase()) || val.mealType[0].toLowerCase().startsWith(input.value.toLowerCase()) || val.cuisine.toLowerCase().startsWith(input.value.toLowerCase());
      });
